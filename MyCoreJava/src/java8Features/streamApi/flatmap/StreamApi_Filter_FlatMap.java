...
public class StreamApi_Filter_FlatMap {
...
//=========================================================================
		...
		List<String> collectedSingleLista = allList.stream().flatMap(nwList -> nwList.stream()).collect(Collectors.toList());	// flatMap "List" lega aur uska "Stream" return karega 
		System.out.println(collectedSingleLista);		// [A, B, C, D, E, F, G]
		
		
		List<String> collectedSingleListb = Stream.of(list1, list2, list3).flatMap(nwList -> nwList.stream()).collect(Collectors.toList());	// flatMap "List" lega aur uska "Stream" return karega 
...
//=========================================================================		
		...
		List<Integer> collectedIntList = Stream.of(intList1, intList2, intList3).flatMap(nwList -> nwList.stream()).collect(Collectors.toList());	
		collectedIntList.stream().filter(n -> n != 3).forEach(i -> System.out.print(i +" "));
		
		System.out.println();
		
		// In single line -
		Stream.of(intList1, intList2, intList3).flatMap(nwList -> nwList.stream()).filter(n -> n != 3).forEach(i -> System.out.print(i +" "));

...
class Hii extends A implements Runnable			// When already inhering some other class, then implementing run() method can only be used by implementing Runnable Interface		
{										
	public void run() 
	{
		for(int i=1; i<=5; i++)
...
//			try { Thread.sleep(500); } catch (Exception e) {e.printStackTrace(); }
			System.out.println(Thread.currentThread().getName() +" isAlive status : "+Thread.currentThread().isAlive());
		}
	}
}

class Helloo implements Runnable		// We can do call run() even even this class extends "Thread" class. Then why implementing Runnable interface ?
...
public class ThreadRunnableDemo {
	...
	public static void main(String[] args) {
		...
		th3.start();
		System.out.println(Thread.currentThread().getName() +"-thread isAlive-status : "+Thread.currentThread().isAlive());

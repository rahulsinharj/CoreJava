...
public class ThreadRunnable_withLambda {

	public static void main(String[] args) throws InterruptedException {
		
		Runnable obj1 = () -> 									// Providing implementation Using Anonymous class , implementing all the methods of Runnable interface here inside.  
					{											// Now since Runnable is functional interface(only single abstract method present in it), so we can also use Lambda expression here. 
						for(int i=1; i<=5; i++)
						{
							System.out.println("HiClass 	-- " +Thread.currentThread().getName() +"	 -- " +Thread.currentThread().getPriority());
		...
		Thread t2 = new Thread(() -> 
					{
						for(int i=1; i<=5; i++)
						{
							System.out.println("HelloClass 	-- " +Thread.currentThread().getName() +"	 -- " +Thread.currentThread().getPriority());
							try { Thread.sleep(1000); } catch (Exception e) {e.printStackTrace(); }
						}
					});			
